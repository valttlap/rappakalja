...
import { GameState, GameStatusDto } from '../models/game-status';
...
export class GameService {
  ...
  public updateGameStatus(status: GameStatusDto): void {
    this.joinCode = status.joinCode;
    this.gameId = status.gameId;
    this.isDasher = status.isDasher;
    this.isOwner = status.isOwner;
    this.status = GameService.gameStateEnumToStatus(status.state);
  }
}
